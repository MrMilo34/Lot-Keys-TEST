...
function paintSelectedListing({followup=false}={}){
  ...
  const follow=$('#followup-section');if(follow){follow.hidden=!(followup||selected?.facebookUrl);follow.classList.toggle('active-followup',!!followup)}
...
const statusTimer=setInterval(()=>{if(!extensionAlive()){clearInterval(statusTimer);return}refreshStatus().catch(()=>{})},2500);
(async()=>{try{const cache=await safeStorageGet(['lotkeysListings','lotkeysThumbCache',POSTING_SESSION_KEY]);thumbCache=cache.lotkeysThumbCache&&typeof cache.lotkeysThumbCache==='object'?cache.lotkeysThumbCache:{};listings=Array.isArray(cache.lotkeysListings)?cache.lotkeysListings:[];postingSession=cache[POSTING_SESSION_KEY]&&typeof cache[POSTING_SESSION_KEY]==='object'?cache[POSTING_SESSION_KEY]:null;selected=null;renderListings();$('#selected-section').hidden=true;$('#followup-section').hidden=true;$('#posting-session-section').hidden=true;if(!extensionAlive()){setStatus(false,false,'Helper updated · reload Facebook');return}await refreshStatus();checkForExtensionUpdate({quiet:true}).catch(()=>{});if(!listings.length)await syncListings();else if(postingSession?.listingId)await restorePostingSession()}catch{setStatus(false,false,'Helper updated · reload Facebook')}})();
