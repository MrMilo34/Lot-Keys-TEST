# LotKeys V0.9.4.100 team-test checklist

## Clean V0.9.4.83 baseline

- Open `https://mrmilo34.github.io/Lot-Keys-TEST/?build=094100` and confirm **V0.9.4.100 / 094100**.
...
- If the stable TEST signing secrets are configured, download **LotKeys-Android-V0.9.4.100** and install its `app-debug.apk` on an Android 11+ test phone. Otherwise confirm CI completes Android assemble/lint and intentionally withholds the unsigned APK.
...
- Confirm unread Important category chips use a black dot in light theme and a white dot in dark theme.
- Read each Device conversation and confirm its category chip and Hub-tab dot disappear when that category's unread total reaches zero.
...
- With a V0.9.4.92 reminder present, open V0.9.4.100 online and confirm it appears once, keeps its title/note/date/time/link/completion state, and no longer participates in appointment collisions after safe sync.
