/* LotKeys Hub V0.9.4.86 — pure models, filtering and appointment utilities. */
(function(root){
'use strict';
...
function validateContact(c){if(!text(c.name))throw Error('Enter a contact name.');const fs=c.fields||[],ps=fs.filter(f=>f.kind==='phone');if(!c.lotkeysId&&!ps.some(f=>validPhone(f.value)))throw Error('Add a valid phone number.');for(const f of ps){if(text(f.value)&&!validPhone(f.value))throw Error('Check '+(f.label||'phone number')+'.');}if(ps.length&&ps.filter(f=>f.primary).length!==1)throw Error('Select one primary number.');if(ps.length&&!validPhone(ps.find(f=>f.primary)?.value))throw Error('The primary phone number must be valid.');for(const f of fs){if(f.kind==='email'&&text(f.value)&&! /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(f.value))throw Error('Check '+(f.label||'email')+'.');}return c;}
...
const api={uid,text,esc,color,phone,validPhone,primary,nextLabel,field,validateContact,matches,filtered,localDay,atLocal,agenda,overlap,monthCells,ics,questions,suggestNotes};root.LotKeysHubCore=api;if(typeof module!=='undefined')module.exports=api;
