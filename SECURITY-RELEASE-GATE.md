...
V0.9.4.95 is suitable for controlled testing with approved Google accounts, the matching explicitly installed Android test APK when the stable TEST signing secrets are configured, and non-sensitive dealership test data. It is **not yet the public-production security finish line**.
...
- V0.9.4.95 does not provide RCS coverage, automatic direct MMS delivery, complete group/dual-SIM handling, iPhone support, or reminder push notifications. Its browser-independent Android relay is a controlled foreground-service TEST implementation rather than production push infrastructure; Android force-stop, battery policy, lost connectivity or revoked Google access can still require reopening the connector. Its media feature is a reviewed handoff to the phone's default messaging app. The UI must continue to show amber partial coverage honestly.
