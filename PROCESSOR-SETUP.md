...
1. Upload the complete V0.9.4.60 website package to GitHub Pages.
...
2. Replace the default `Code.gs` contents with the complete contents of `processor/Code.gs` from this release.
3. Open **Project Settings**, enable **Show “appsscript.json” manifest file in editor**, then replace that manifest with `processor/appsscript.json` from this release.
...
5. From the function menu select `installLotKeysProcessor`, press **Run**, review the requested Google permissions, and approve them.
