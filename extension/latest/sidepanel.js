...
function setFollowupSourceLocked(locked=true){document.body.classList.toggle('followup-source-locked',!!locked);for(const section of [$('#listing-picker-section'),$('#selected-section')]){if(!section)continue;section.inert=!!locked;section.setAttribute('aria-disabled',locked?'true':'false')}}
function paintSelectedListing({followup=false,sourceLocked=null}={}){
  ...
  const follow=$('#followup-section');if(follow){follow.hidden=!(followup||selected?.facebookUrl);follow.classList.toggle('active-followup',!!followup);setFollowupLocked(!selected?.facebookUrl);setFollowupSourceLocked(sourceLocked==null?!!followup:!!sourceLocked)}
...
function showPickerStart({keepSelected=false}={}){
  stopFieldWatch();setFollowupSourceLocked(false);$('#fill-facebook').hidden=false;$('#followup-section').classList.remove('active-followup');
...
function showPostingSession(phase='filling',{restore=false}={}){
  if(!selected)return;setFollowupSourceLocked(false);$('#listing-picker-section').hidden=true;$('#selected-section').hidden=true;$('#followup-section').hidden=true;$('#posting-session-section').hidden=false;paintSessionCard();
...
async function enterFollowupView({restore=false}={}){
  if(!selected)return;stopFieldWatch();const websiteAlreadySaved=restore&&String(postingSession?.phase||'')==='website_saved';if(!websiteAlreadySaved)await savePostingSession({listingId:selected.id,phase:'followup',result:lastResult,startedAt:postingSession?.startedAt||Date.now(),updatedAt:Date.now()});
  $('#posting-session-section').hidden=true;$('#progress-section').hidden=true;$('#listing-picker-section').hidden=false;renderListings();paintSelectedListing({followup:true,sourceLocked:!websiteAlreadySaved});if(!restore)setTimeout(()=>$('#followup-section')?.scrollIntoView({behavior:'smooth',block:'start'}),70);
}
async function returnToTransferComplete(){if(!selected)return;setFollowupSourceLocked(false);await savePostingSession({...postingSession,listingId:selected.id,phase:'transfer_complete',result:lastResult,startedAt:postingSession?.startedAt||Date.now(),updatedAt:Date.now()});showPostingSession('transfer_complete',{restore:true})}
...
async function selectListing(id){
  if(postingSession?.listingId&&String(postingSession.listingId)!==String(id))await clearPostingSession();stopFieldWatch();setFollowupSourceLocked(false);selected=listings.find(x=>x.id===id)||null;renderListings();$('#posting-session-section').hidden=true;$('#listing-picker-section').hidden=false;
